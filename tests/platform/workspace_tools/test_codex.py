"""
Platform workspace tools tests for Codex agent.

Tests that the MCP server correctly exposes workspace tools that
Codex would use (via system prompt injection of HTTP endpoints).
Verifies the tool registry since the same MCP server powers all
agent types.

Run:
    pytest tests/platform/workspace_tools/test_codex.py -v
"""
...
class TestCodexWorkspaceTools:
    """Test workspace tool availability for Codex."""
...
class TestCodexWorkspaceToolsReport:
