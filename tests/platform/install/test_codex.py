"""
Platform install tests for Codex agent.

Tests the real user experience: `openagents install codex`
across Linux, macOS, and Windows.

Run:
    pytest tests/platform/install/test_codex.py -v
"""
...
class TestCodexInstall:
    """Test installing Codex via `openagents install codex`."""
    ...
    def test_openagents_install_codex(self):
        """`openagents install codex --yes` should succeed."""
        try:
    ...
    def test_binary_on_path(self):
        """After install, 'codex' binary should be on PATH."""
    ...
    def test_binary_version(self):
        """'codex --version' should return a version string."""
    ...
    def test_binary_help(self):
        """'codex --help' should print usage info."""
        if shutil.which(BINARY_NAME) is None:
            pytest.skip(f"'{BINARY_NAME}' not on PATH")
