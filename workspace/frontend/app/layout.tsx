...
import { DESKTOP_PREPAINT_SCRIPT } from '@/lib/desktop';
import { SIDEBAR_WIDTH_PREPAINT_SCRIPT } from '@/lib/panel-store';
  ...
  return (
    <html lang="en" suppressHydrationWarning>
      <head>
        ...
        {/* Stamps data-desktop / data-platform on <html>. Must also beat first
            paint: it is what sets --titlebar-height and the window-control
            insets, so deferring it would lay the shell out web-shaped for a
            frame and then jump. */}
        <script dangerouslySetInnerHTML={{ __html: DESKTOP_PREPAINT_SCRIPT }} />
        {/* Sets --sidebar-width on <html> before first paint so the layout settles
            at the user's stored width immediately with no width snap. */}
        <script dangerouslySetInnerHTML={{ __html: SIDEBAR_WIDTH_PREPAINT_SCRIPT }} />
        {POSTHOG_KEY && (
