...
func ReportAgentUsage(c *gin.Context) {
	...
	exists := db.DB.Where("workspace_id = ? AND agent_name = ?", workspace.ID, agentName).First(&record).Error == nil

	record.WorkspaceID = workspace.ID
	record.AgentName = agentName
	record.SessionUsedPercent = req.SessionUsedPercent
	record.SessionResetsAt = req.SessionResetsAt
	record.WeekUsedPercent = req.WeekUsedPercent
	record.WeekResetsAt = req.WeekResetsAt
	record.Last24hSummary = req.Last24hSummary
	record.Last7dSummary = req.Last7dSummary
	record.CurrentModel = req.CurrentModel
	record.AvailableModels = req.AvailableModels
	record.CurrentEffort = req.CurrentEffort
	record.AvailableEfforts = req.AvailableEfforts
	record.RawText = req.RawText

	if req.TotalPromptTokens != nil {
		record.TotalPromptTokens = *req.TotalPromptTokens
	}
	if req.TotalCompletionTokens != nil {
		record.TotalCompletionTokens = *req.TotalCompletionTokens
	}
	if req.TotalTokens != nil {
		record.TotalTokens = *req.TotalTokens
	} else if req.TotalPromptTokens != nil || req.TotalCompletionTokens != nil {
