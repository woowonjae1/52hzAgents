...
class WorkspaceClient {
  ...
  async createWorkspace({ agentName, name, agentType } = {}) {
    ...
    const frontendUrl = (process.env.WWJ_WORKSPACE_UI_URL || this.endpoint)
  ...
  /**
   * Send heartbeat via POST /v1/workspaces/:workspace_id/presence.
   *
   * @param {string} sessionId - session id returned by /v1/join.
   ...
   */
  async heartbeat(workspaceId, agentName, token, sessionId, status = 'online', reason = '') {
    if (!sessionId) {
      throw new Error('session_id is required for workspace presence updates');
    }
    const body = {
      agent_name: agentName,
    ...
    const data = await this._post(
      `/v1/workspaces/${encodeURIComponent(workspaceId)}/presence`,
      body,
