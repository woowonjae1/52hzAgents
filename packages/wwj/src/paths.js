...
function getExtraBinDirs() {
  ...
  // Add portable Node.js directory (~/.wwj/nodejs/)
  const portableNode = path.join(HOME, '.wwj', 'nodejs');
  _push(dirs, portableNode);

  // Core library bin (~/.wwj/core/node_modules/.bin)
  _push(dirs, path.join(HOME, '.wwj', 'core', 'node_modules', '.bin'));

  // Per-agent runtime bins (~/.wwj/runtimes/<type>/node_modules/.bin)
  const runtimesDir = path.join(HOME, '.wwj', 'runtimes');
  try {
...
function _addUnixPaths(dirs) {
  ...
  // npm agents install to isolated prefixes: ~/.wwj/runtimes/<type>/
...
function getRuntimePrefix(agentType) {
  return path.join(HOME, '.wwj', 'runtimes', agentType);
}
...
function getCorePrefix() {
  return path.join(HOME, '.wwj', 'core');
}
 ...
 *
 * Rooted under ~/.wwj — the same writable tree the daemon already uses
...
function defaultAgentWorkdir(agentName) {
  ...
  const dir = path.join(os.homedir(), '.wwj', 'workspaces', safe);
