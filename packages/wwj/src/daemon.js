...
class Daemon {
  ...
  static daemonize(configDir, foregroundArgs, execPath) {
    ...
    // Without this check, repeated `wwj up` invocations would spawn
    ...
    if (existingPid) {
      ...
      console.error(`Run 'wwj down' first, or 'wwj status' to check.`);
      process.exit(1);
    }
  ...
  static stopDaemon(configDir) {
    ...
    // `wwj down` would signal a dead pid, delete the files, report "Daemon
    // stopped", and leave the real daemon orphaned — which then blocked every
    // subsequent `wwj up` ("already running") and kept its agents wedged.
    ...
    // Always clear BOTH sources of truth so a fresh `wwj up` starts clean.
    try { fs.unlinkSync(pidFile); } catch {}
  ...
  static readDaemonPid(configDir) {
    ...
    // pid file missing or stale — fall back to the status file so `wwj status`
    // reports the SAME live daemon the `wwj up` singleton guard detects.
  ...
  async _adapterLoop(name, agentCfg, info, network) {
    ...
    const endpoint = network.endpoint || process.env.WWJ_WORKSPACE_ENDPOINT || process.env.WWJ_ENDPOINT || 'http://localhost:8000';
  ...
  _buildAgentEnv(agentCfg) {
    ...
    // A local Codex CLI should use `codex login` unless this launcher has an
    // explicit API configuration for Codex. Do not inherit unrelated keys or
    // proxy URLs from the launcher process.
    if (type === 'codex') {
   ...
   * file's. Used by the singleton guard, `wwj status`, and `wwj down` so every
