...
class Installer {
  ...
  _aiderBinaryNotFoundMessage() {
    ...
    return (
      ...
      '     (the install dir may simply not be on this processâ€™s PATH yet)\n' +
  ...
  _buildShellEnv() {
    ...
    // CreateProcess on it, which surfaces as `cmd: æ‹’ç»è®¿é—®` ("Access
   ...
   * `node npm-cli.js â€¦` with argv passed array-style and NO shell. On Windows
   * that goes through CreateProcessW (UTF-16), so an install prefix under a
   * non-ASCII home dir (e.g. `C:\Users\ç”¨æˆ·å\.wwj\runtimes\â€¦`) is
