 ...
 *   1. ~/.wwj/installed_agents.json  (JSON array of names)
 *   2. ~/.wwj/installed/<name>       (empty marker files)
 */
class Installer {
   ...
   *   - managed: true if installed inside ~/.wwj/ (can be uninstalled by launcher)
  ...
  getInstallInfo(agentType) {
    ...
    // Check isolated runtime prefix first (~/.wwj/runtimes/<type>/)
    ...
    // Legacy: check shared prefix (~/.wwj/nodejs/node_modules/)
    const legacyModules = path.join(os.homedir(), '.wwj', 'nodejs', 'node_modules');
    ...
    if (!binaryPath) {
      ...
      // when binary detection can't (yet) see the freshly-installed CLI �?      // e.g. PATH caches not yet picking up a brand-new ~/.cursor/bin. The
    ...
    const hzAgentsDir = path.join(os.homedir(), '.wwj');
    if (binaryPath.startsWith(hzAgentsDir)) {
    ...
    // Binary found outside ~/.wwj/ �?global/system install
  ...
  _verifyAmpBinary() {
    ...
    if (resolved) candidates.push(resolved);
    // 2) Honor a custom AMP_HOME, then the installer's default ~/.amp/bin �?    //    covers a binary that exists but whose dir isn't on PATH yet.
    const binDirs = [];
  ...
  /**
   * Existence/readability of a credential file WITHOUT reading its contents �?   * never loads a token/key into memory. Returns 'present' | 'unreadable' |
  ...
  _cleanStaleShims(agentType) {
    ...
    const legacyDir = path.join(os.homedir(), '.wwj', 'nodejs');
  ...
  _resolvePackageBin(agentType, entry, binary) {
    ...
    const prefixes = [
      ...
      path.join(os.homedir(), '.wwj', 'nodejs', 'node_modules'),
    ];
  ...
  hasNodejs() {
    ...
    // Check bundled Node.js in ~/.wwj/nodejs/
    if (process.platform === 'win32') {
      try {
        const bundledDir = path.join(this.configDir, 'nodejs');
        if (fs.existsSync(bundledDir)) {
  ...
  async installNodejs(onData) {
    ...
    if (plat === 'windows') {
      ...
      // macOS / Linux: download portable tar.gz/tar.xz, extract to ~/.wwj/nodejs/
   ...
   * non-ASCII home dir (e.g. `C:\Users\用户名\.wwj\runtimes\…`) is
  ...
  _resolveNodeNpmCli() {
    ...
    // Bundled portable node (~/.wwj/nodejs, plus nested node-* layouts)
    const bundledDir = path.join(this.configDir, 'nodejs');
