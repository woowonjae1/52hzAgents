/**
 * Interactive TUI dashboard for 52hzAgents — `openagents` or `openagents tui`
 *
...
function getConnector() {
  const configDir = path.join(process.env.HOME || process.env.USERPROFILE || '.', '.wwj');
...
function createTUI() {
  ...
  function showConfigureScreen(agent) {
    ...
    blessed.box({
      ...
      tags: true,
      style: { bg: COLORS.headerBg, fg: COLORS.headerFg, bold: true },
      content: `  {bold}Configure ${agent.type}{/bold}  {gray-fg}\u2014  Saved to ~/.wwj/env/{/gray-fg}`,
    });
  ...
  function showConnectWorkspaceScreen(agentName) {
    ...
    for (const net of networks) {
      ...
      const workspaceUI = net.uiEndpoint || process.env.WWJ_WORKSPACE_UI_URL || net.endpoint;
      const url = `${String(workspaceUI || '').replace(/\/$/, '')}/${slug}`;
  ...
  function doOpenWorkspace(agent) {
    ...
    const slug = net.slug || net.id;
    const workspaceUI = net.uiEndpoint || process.env.WWJ_WORKSPACE_UI_URL || net.endpoint;
    let url = `${String(workspaceUI || '').replace(/\/$/, '')}/${slug}`;
