...
/**
 * MCP (Model Context Protocol) server for WWJ workspace tools.
 *
 ...
 *
 * Usage:
 *   wwj mcp-server --workspace-id <id> --channel-name <ch> --agent-name <name>
 *
 * The workspace token is read from the WWJ_WORKSPACE_TOKEN env var.
 */
    ...
    switch (method) {
      case 'initialize':
        this._write(jsonRpcResponse(id, {
          ...
          capabilities: { tools: {} },
          serverInfo: { name: 'wwj-workspace', version: '1.0.0' },
        }));
        break;
      case 'tools/list':
