...
class CursorAdapter extends BaseAdapter {
  ...
  _findCursorBinary() {
    ...
    for (const name of names) {
      const runtimeCandidate = path.join(home, '.wwj', 'runtimes', 'cursor', 'node_modules', '.bin', `${name}${ext}`);
      if (fs.existsSync(runtimeCandidate)) return runtimeCandidate;
    }

    // Tier 0b: Legacy portable install
    for (const name of names) {
      const portableCandidate = path.join(home, '.wwj', 'nodejs', 'node_modules', '.bin', `${name}${ext}`);
