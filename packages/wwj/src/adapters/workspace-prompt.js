 ...
 * - `'skills'`  -> points at the wwj-workspace skill (Bash + curl),
...
function buildApiSkillsPrompt({ endpoint, workspaceId, token, agentName, channelName, disabledModules, mode = 'execute', isWindows = process.platform === 'win32' }) {
  ...
  if (!isPlan && !disabled.has('timers')) {
    ...
    sections.push(
      ...
      'yourself to follow up, deliver a one-off reminder at a time the user named.\n\n' +

      'A timer fires **once**. For anything that repeats, use a Routine instead.\n\n' +

      '**Create a timer after a delay** (`delay_seconds`, minimum 1):\n' +

      `\`${curl} -s -X POST -H "${h}" -H "Content-Type: application/json" ` +

      `${baseUrl}/v1/timers -d '{"delay_seconds":300,"message":"Check the build",` +

      `"network":"${workspaceId}","channel":"${channelName}",` +

      `"source":"52hz:${agentName}"}'\`\n\n` +

      '**Create a timer at a specific moment** (`fires_at`, RFC3339 with offset). ' +

      'Use this whenever the user names a clock time - "today at 15:16", ' +

      '"tomorrow 9am", "\u4eca\u5929 15:16", "\u660e\u5929\u4e0a\u5348\u4e5d\u70b9". Pass the moment through directly; ' +

      'do not do the subtraction into a delay yourself:\n' +

      `\`${curl} -s -X POST -H "${h}" -H "Content-Type: application/json" ` +

      `${baseUrl}/v1/timers -d '{"fires_at":"2026-09-08T15:16:00+08:00",` +

      `"message":"Meeting time - you asked me to remind you",` +

      `"network":"${workspaceId}","channel":"${channelName}",` +

      `"source":"52hz:${agentName}"}'\`\n\n` +

      'Give exactly one of `fires_at` or `delay_seconds`. The response echoes the ' +

      'resolved `fires_at`; confirm that value back to the user instead of ' +

      'restating what they asked for.\n\n' +
 ...
 * - `'skills'`  -> the wwj-workspace skill (Bash + curl); no MCP server.
