...
function generateSessionTitle(message, maxWords = 6, maxWidth = 32) {
  ...
  if (displayWidth(text) > maxWidth) {
    ...
    if (!hasCjk) {
      ...
      // than "成都未来7天的天气怎么样" despite carrying two more characters —
      // the dangling fragment reads as breakage. Skipped when the boundary
  ...
  // character is a no-op, so this is skipped rather than harmlessly wasted —
  // it would otherwise also uppercase a mixed title's Latin lead-in against
