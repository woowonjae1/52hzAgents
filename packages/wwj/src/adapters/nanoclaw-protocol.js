 ...
 * format, id/addressing rules, secret redaction, and error classification �? * everything that can be unit-tested without any IO.
 *
 * Wire format: one JSON object per line ("\n"-delimited), both directions.
 *
 *   52hzAgents adapter (client) �?channel (server):
 ...
 *     { op: "ping" }
 *
 *   channel (server) �?52hzAgents adapter (client):
 ...
 *     { op: "pong" }
 *
 * Local trust boundary: `data/52hzAgents/bridge.sock` (0600) lives in a 0700
 * dir; the client must present a random per-host `secret` (read from the 0600
 * `data/52hzAgents/secret` file written by the channel) in `hello`. No frame
 ...
 * Distinct 52hzAgents channels �?distinct platform ids �?distinct NanoClaw
...
function buildInbound({ platformId, threadId = null, msgId, turnId, text, sender = 'user', senderId, ts }) {
  return encodeFrame({
    ...
    msgId,
    // turnId identifies the 52hzAgents delivery turn; the channel echoes it on
...
const _ERR_MESSAGES = {
  ...
  [ERR.CHANNEL_NOT_LOADED]:
    'The NanoClaw `52hzAgents` channel is not loaded. Install it (/add-52hzAgents) and restart the NanoClaw host.',
  [ERR.WIRING_MISSING]:
    'This 52hzAgents channel is not wired to the Agent Group yet. Approve the wiring in NanoClaw (creating wirings requires approval).',
  [ERR.CREDS_MISSING]:
    'NanoClaw is missing its provider credentials. Configure the agent group provider (e.g. Anthropic) in NanoClaw.',
  [ERR.AUTH_FAILED]:
    'Could not authenticate to the NanoClaw 52hzAgents channel (local secret missing or mismatched). Restart the NanoClaw host so a fresh secret is issued.',
  [ERR.INCOMPATIBLE_VERSION]:
    'This NanoClaw version is not compatible with the 52hzAgents channel. Use a verified NanoClaw version, or upgrade the 52hzAgents integration.',
