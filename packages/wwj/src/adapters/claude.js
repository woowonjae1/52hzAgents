...
class ClaudeAdapter extends BaseAdapter {
   ...
   * A message like "@claude 你对以上分析怎么�? then resolves "以上" against
  ...
  _formatClaudeError(text) {
    ...
    if (/401|403|authenticate|invalid.*(token|key|api)|无效�?\s*(令牌|密钥|key)/i.test(msg)) {
  ...
  async _handleMessage(msg) {
    ...
    // relay then rejects every request as 401 "invalid token / 无效的令�?. When a
