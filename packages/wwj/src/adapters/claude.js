...
class ClaudeAdapter extends BaseAdapter {
  ...
  constructor(opts) {
    ...
    this._channelSessions = {}; // channel 鈫?Claude CLI session_id
    // channel 鈫?messageId of the last channel message already accounted for in
    ...
    this._recapCursor = {};
    this._channelProcesses = {}; // channel 鈫?child process
    ...
    this._persistentProcs = {}; // channel 鈫?{ proc, lineBuffer, pendingLines, idleTimer, messageResolve }
    ...
    // rather than folded into the sessions file, which is a bare channel鈫抜d map
  ...
  async _listModels() {
    ...
    for (const tier of await this._listAliasTiers()) {
      ...
      push(tier, target ? `${name} 鈫?${target}` : name);
    }
  ...
  async fetchAndReportUsage(force = false) {
    ...
    try {
      ...
      const sessionMatch = text.match(/Current session:\s*(\d+)%?\s*used(?:\s*路\s*resets\s*([^\n]+))?/i);
      if (sessionMatch) {
      ...
      const weekMatch = text.match(/Current week[^:]*:\s*(\d+)%?\s*used(?:\s*路\s*resets\s*([^\n]+))?/i);
      if (weekMatch) {
      ...
      const h24Match = text.match(/Last 24h\s*路\s*([^\n]+)/i);
      const d7Match = text.match(/Last 7d\s*路\s*([^\n]+)/i);
    ...
    if (action === 'set_model') {
      ...
      if (model) {
        if (channel) {
          ...
          // while the UI chip 鈥?backed by localStorage 鈥?kept showing it, so the
      ...
      if (channel) {
        ...
        try {
          await this.client.sendMessage(this.workspaceId, channel, this.token,
            'Session restarted 鈥?next message starts fresh.',
            {
              senderType: 'agent',
        ...
        // Defensive 鈥?no channel, clear everything before the bounce.
        this._channelSessions = {};
        try { this._saveSessions(); } catch {}
        await this._stopAllProcesses('Execution stopped by user.');
        this._log('Restart: cleared all sessions (no channel param)');
      }
      // Ask the daemon to bounce just THIS agent 鈥?true process-level
   ...
   * the channel's last event as a `status` (e.g. "Bash 鈥?..." mid-tool-call)
   * forever 鈥?the workspace UI then shows the thread as "running" until a
  ...
  stop() {
    ...
    this._stopAllProcesses(
      'Task interrupted 鈥?daemon restarting. Send another message to continue.'
    ).catch(() => {});
    super.stop();
  }
   ...
   * addressing filter), so everything else said in the channel 鈥?including
   * whole analyses posted by sibling agents 鈥?never reaches the CLI session.
   * A message like "@claude 浣犲浠ヤ笂鍒嗘瀽鎬庝箞鐪? then resolves "浠ヤ笂" against
   ...
   *   old behaviour 鈥?a tail recap of the recent conversation, own messages
  ...
  async _buildChannelContext(channelName, opts = {}) {
    ...
    if (cursor) {
      ...
      if (idx === -1) {
        // Cursor aged out of the window 鈥?fall back to a tail recap rather
    ...
    for (let i = startIdx; i < messages.length; i++) {
      ...
      if (!text) continue;
      // Exclude the message being handled 鈥?the caller appends it below.
      ...
      const who = m.senderType === 'human'
        ? (m.senderName || 'user')
        : (m.senderName || 'agent');
      const truncated = text.length > 2000 ? text.slice(0, 2000) + '鈥? : text;
    ...
    return (
      ...
      'it means the channel messages above 鈥?not your own earlier work.'
    );
  }
  ...
  _resolveToNodeCmd(binPath) {
    ...
    if (IS_WINDOWS && binPath.toLowerCase().endsWith('.cmd')) {
      ...
      // claude.cmd 鈫?@anthropic-ai/claude-code/bin/claude.exe). Resolve to the
      ...
      // Unix: symlink 鈫?resolve to actual .js file
      try {
        let target = binPath;
  ...
  _findClaudeBinary() {
    ...
    // launcher adds 鈥?that's why a bare `which claude` came up empty before.
    ...
    // homebrew, cargo, pip, 鈥?. This is what catches a `claude` installed as a
    // global npm package under a version-managed Node 鈥?the most common setup,
  ...
  _buildClaudeCmd(prompt, channelName, { skipResume = false, browserEnabled = false, workingDir } = {}) {
    ...
    // (mcp 鈫?workspace_* tools, skills 鈫?wwj-workspace skill), so there
    ...
    // 鈹�鈹� Skills mode: write SKILL.md, no MCP server 鈹�鈹�
    if (this.toolMode === 'skills') {
      return this._buildSkillsCmd(cmd, channelName, workingDir);
    }

    // 鈹�鈹� MCP mode (default): spawn MCP server 鈹�鈹�
  ...
  _buildSkillsCmd(cmd, channelName, workingDir) {
    ...
    // process.cwd() as the fallback 鈥?on a packaged Windows daemon that is
  ...
  _buildMcpCmd(cmd, channelName) {
    ...
    if (fs.existsSync(siblingBin)) {
      ...
      if (!oaBin) {
        this._log('Could not find wwj binary 鈥?MCP tools may not be available');
        mcpCommand = 'wwj';
      } else {
  ...
  _startWatchdog(pp) {
    ...
    pp.watchdogTimer = setInterval(async () => {
      ...
      if (consecutiveTimeouts >= this._WATCHDOG_MAX_TIMEOUTS) {
        this._log(`Watchdog: process unresponsive for ${consecutiveTimeouts * 15}s on ${pp.msgChannel} 鈥?killing`);
        this._stopWatchdog(pp);
        try { await this.sendError(pp.msgChannel, `鈿狅笍 Agent execution exceeded watchdog limit (${Math.round((consecutiveTimeouts * 15) / 60)}m) and was restarted.`); } catch {}
  ...
  _spawnPersistentProc(channel, cmd, cleanEnv, workingDir) {
    // Remove -p and its argument from cmd 鈥?prompts go via stdin
    const filteredCmd = [];
    for (let i = 0; i < cmd.length; i++) {
      if (cmd[i] === '-p' || cmd[i] === '--print') {
        // -p in stream-json mode is just a flag (no argument to skip)
        // but _buildClaudeCmd passes [-p, prompt] 鈥?skip both
      ...
      if (eventType === 'assistant') {
        ...
        for (const block of blocks) {
          if (block.type === 'text' && block.text && block.text.trim()) {
             ...
             * through this if/else chain and be discarded 鈥?so the model with
            ...
            await this.sendStatus(pp.msgChannel, `${toolName} 鈥?${inputPreview}`);
            pp.everPostedAnything = true;
          }
        }
        ...
        if (pp.messageResolve) {
          ...
          // streamed as `thinking` only 鈥?with no messageResolve waiting,
          // _handleMessage never posts it as a `chat`, so the thread visually
          // hangs on "thinking鈥?Working鈥?. Finalize the accumulated text as a
   ...
   * invisible 鈥?add a concrete hint about which env vars to check.
   */
  _formatClaudeError(text) {
    const msg = String(text || '').trim() || 'Claude returned an error.';
    if (/401|403|authenticate|invalid.*(token|key|api)|鏃犳晥鐨?\s*(浠ょ墝|瀵嗛挜|key)/i.test(msg)) {
      return `Claude authentication failed: ${msg}\n\n` +
        'Check this agent\'s API key and Base URL in the launcher 鈥?the key may be invalid or expired.';
    }
   ...
   *
   * Never rejects 鈥?a failed upload must not affect the conversation.
   *
  ...
  async _handleMessage(msg) {
    ...
    // Resolved once per message (not stashed on `this`) 鈥?multiple channels can
    ...
    // 鈹�鈹� Persistent process fast-path 鈹�鈹�
    ...
    if (existingPP && existingPP.alive) {
      ...
      // The live session only ever saw messages addressed to us 鈥?push in
      ...
      if (result.resultEvent) {
        ...
        if (!msg._todoNudge) {
          ...
          // still on the panel. Cancel it 鈥?the cursor adapter has always done
      ...
      // Process died mid-message 鈥?fall through to spawn a fresh one
    ...
    // path 鈫?403). But preserve config vars the child needs for cloud
    ...
    // ANTHROPIC_API_KEY the CLI sends `x-api-key`, which most relays ignore 鈥?the
    // relay then rejects every request as 401 "invalid token / 鏃犳晥鐨勪护鐗?. When a
