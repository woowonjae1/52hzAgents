...
class ClineAdapter extends BaseAdapter {
  ...
  _findClineBinary() {
    ...
    // Tier 0: isolated runtime prefix (~/.wwj/runtimes/cline/)
    const runtimeCandidate = path.join(home, '.wwj', 'runtimes', 'cline', 'node_modules', '.bin', `cline${ext}`);
    if (fs.existsSync(runtimeCandidate)) return runtimeCandidate;

    // Tier 0b: legacy portable install
    const portable = path.join(home, '.wwj', 'nodejs', 'node_modules', '.bin', `cline${ext}`);
    ...
    // prefix.
    for (const root of [
      path.join(home, '.wwj', 'runtimes', 'cline', 'node_modules', 'cline'),
      path.join(home, '.wwj', 'nodejs', 'node_modules', 'cline'),
    ]) {
