 ...
 *
 * Bridges the OFFICIAL GitHub Copilot CLI â€?executable `copilot`, npm package
 * `@github/copilot` â€?to an 52hzAgents workspace. This is NOT the retired
 ...
 * blindly):
 *   â€?Auth is GitHub-based (env tokens / gh / keychain OAuth) â€?never read or
 *     log tokens; the launcher/installer reports auth state, the CLI's own run
 *     result is the final authority.
 *   â€?Permissions are explicit and least-privilege: scoped to the working dir
 ...
 *   â€?plan mode â†?analysis only (--plan, no write tools); act mode â†?controlled
 *     read/write/shell within the working dir.
 *
 * â”€â”€ Verification status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// stably provide everything the adapter relies on â€?`--output-format=json`
...
const SECRET_ENV_VARS = [
  'COPILOT_GITHUB_TOKEN', 'GH_TOKEN', 'GITHUB_TOKEN', 'WWJ_WORKSPACE_TOKEN',
];
...
class CopilotAdapter extends BaseAdapter {
   ...
   * being resumed against another (the spec's "Session ä¸?working directory
   * ç»‘å®š" / no cross-project resume requirement).
   */
  ...
  _findCopilotBinary() {
    ...
    // Tier 0: isolated runtime prefix (~/.wwj/runtimes/copilot/.bin)
    let hit = named(path.join(home, '.wwj', 'runtimes', 'copilot', 'node_modules', '.bin'));
    if (hit) return hit;
    // Tier 0b: legacy portable npm prefix
    hit = named(path.join(home, '.wwj', 'nodejs', 'node_modules', '.bin'));
    if (hit) return hit;
   ...
   * argv element (notably the prompt) literal â€?no cmd.exe re-parsing, so no
  ...
  _resolveExec(bin, isWindows = IS_WINDOWS) {
    ...
    // spawn a `.js` (no shebang support, not a PE â†?`spawn UNKNOWN`); on Unix a
    ...
    // Native executable â€?spawn directly (shell:false keeps argv literal).
    if (lower.endsWith('.exe')) return [bin];

    // npm/batch shim: resolve to its real target so we spawn shell:false and
    // every argv element (notably the prompt) stays literal â€?no cmd.exe
    ...
    // No extension (e.g. a Unix-style shim copied to Windows) â€?spawn directly.
    return [bin];
  }
  ...
  // ------------------------------------------------------------------
  // Command construction (argv array â€?never a shell string)
  // ------------------------------------------------------------------
  ...
  _buildArgs(prompt, channel, { skipResume = false } = {}) {
    ...
    // (--resume, --secret-env-vars, --allow-tool â€?declared `[=value]` /
    ...
    if (this._mode === 'plan') {
      // Analysis/planning only â€?do not grant write capability.
      args.push('--plan');
    } else {
  ...
  /**
   * Graceful â†?forceful interrupt of a Copilot subprocess and its whole process
  ...
  async _handleMessage(msg) {
    ...
    if (gate.compatible === false) {
      await this.sendError(channel,
        `GitHub Copilot CLI ${gate.version} is too old â€?this integration requires ${MIN_VERSION} or newer. Upgrade with: copilot update (or npm install -g @github/copilot).`);
      return;
    }

    // Working directory must exist â€?never silently fall back to the repo cwd.
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      // Stale resume â†?retry once without it.
   ...
   *   â€?compatible:false â†?below MIN_VERSION, callers must not spawn.
   *   â€?compatible:null  â†?version unparseable/undetectable ("unknown"); we do
   ...
   * the turn. Never rejects on CLI failure â€?failures come back as
      ...
      const handle = async (ev) => {
        switch (ev.kind) {
          ...
          case 'text_delta':
            ...
            // once that authoritative answer arrives â€?the `reasoning` case
          ...
          case 'file_change':
            ...
            break;
          case 'tool_result':
            // Tool output is operational detail, not an assistant reply â€?keep it
  ...
  _describeTool(ev) {
    ...
    return `${ev.tool} â€?${redactSensitive(String(preview))}`;
  }
   ...
   * test/fixtures/copilot-cli-real-samples.md) â€?note auth/session/network
  ...
  _classifyOutcome(exitCode, errorEvent, stderr) {
    ...
    // Session resume miss â†?retry fresh (not a user-facing error).
    ...
    // Token present but REJECTED â€?must be checked BEFORE the no-credentials rule
    // because the real CLI appends the SAME "To authenticate â€?gh auth login"
