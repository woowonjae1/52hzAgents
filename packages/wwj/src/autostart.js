...
const SERVICE_LABEL = 'org.wwj.connector';
...
function isEnabled() {
  ...
  if (IS_LINUX) {
    const unitPath = path.join(HOME, '.config', 'systemd', 'user', 'wwj-launcher.service');
    return fs.existsSync(unitPath);
  }
  if (IS_WINDOWS) {
    try {
      execSync(`schtasks /Query /TN "WWJ Launcher"`, { stdio: 'pipe', timeout: 5000 });
      return true;
    } catch {
      return false;
    }
...
function _enableLinux(nodeBin, cliPath, configDir) {
  ...
  const unitPath = path.join(unitDir, 'wwj-launcher.service');

  const unit = `[Unit]
Description=WWJ Launcher Daemon
...
`;
  ...
  execSync('systemctl --user enable wwj-launcher.service', { stdio: 'pipe', timeout: 5000 });
  execSync('systemctl --user start wwj-launcher.service', { stdio: 'pipe', timeout: 5000 });
  return { enabled: true, path: unitPath };
}

function _disableLinux() {
  try {
    execSync('systemctl --user stop wwj-launcher.service', { stdio: 'pipe', timeout: 5000 });
  } catch {}
  try {
    execSync('systemctl --user disable wwj-launcher.service', { stdio: 'pipe', timeout: 5000 });
  } catch {}
  const unitPath = path.join(HOME, '.config', 'systemd', 'user', 'wwj-launcher.service');
...
function _disableWindows() {
  try {
    execSync('schtasks /Delete /TN "WWJ Launcher" /F', { stdio: 'pipe', timeout: 5000 });
  } catch {}
  return { enabled: false };
}
