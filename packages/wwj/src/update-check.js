 ...
 *
 * Queries the npm registry for the latest WWJ launcher package.
...
const PKG_NAME = process.env.WWJ_NPM_PACKAGE || 'wwj';
const CACHE_FILE = path.join(os.homedir(), '.wwj', '.update-check.json');
 ...
 * from `<dir>/node_modules` — so `wwj update` would report success while the
 ...
 * Never prompts or auto-updates — users run `wwj update` explicitly.
 */
