...
function getConnector(flags) {
  ...
  if (flags.endpoint || process.env.WWJ_WORKSPACE_ENDPOINT || process.env.WWJ_ENDPOINT) {
    opts.workspaceEndpoint = flags.endpoint || process.env.WWJ_WORKSPACE_ENDPOINT || process.env.WWJ_ENDPOINT;
  }
...
async function cmdStatus(connector) {
  ...
  if (agents.length === 0) {
    print('\nNo agents configured. Run: wwj create <name> --type <type>');
    return;
  }
...
async function cmdCreate(connector, flags, positional) {
  ...
  if (!connector.registry.getEntry(type)) {
    ...
    print('Run `wwj search` to browse available agent types.');
    process.exitCode = 1;
    return;
  }
  ...
  try {
    ...
    if (created && !created.network) {
      ...
      print(`  wwj connect ${name} <workspace-token>`);
    } else {
...
async function cmdRemove(connector, _flags, positional) {
  const name = positional[0];
  if (!name) { print('Usage: wwj remove <name>'); return; }
...
async function cmdStop(connector, _flags, positional) {
  const name = positional[0];
  if (!name) { print('Usage: wwj stop <name>'); return; }
...
async function cmdInstall(connector, _flags, positional) {
  ...
  if (!type) { print('Usage: wwj install <type>'); return; }
...
async function cmdUninstall(connector, _flags, positional) {
  const type = positional[0];
  if (!type) { print('Usage: wwj uninstall <type>'); return; }
...
async function cmdDisconnect(connector, _flags, positional) {
  const name = positional[0];
  if (!name) { print('Usage: wwj disconnect <agent-name>'); return; }
...
async function cmdWorkspace(connector, flags, positional) {
  ...
  switch (sub) {
    ...
    case 'join': {
      ...
      if (!token) { print('Usage: wwj workspace join <token>'); return; }
      try {
        const info = await connector.resolveToken(token);
        connector.config.addNetwork({
          id: info.workspace_id,
...
async function cmdEnv(connector, flags, positional) {
  const type = positional[0];
  if (!type) { print('Usage: wwj env <type> [--set KEY=VALUE]'); return; }
...
async function cmdToolMode(connector, _flags, positional) {
  ...
  if (!first) {
    ...
    print('\nUsage: wwj tool-mode <agent|--all> <mcp|skills>');
    return;
  }
...
async function cmdSkills(connector, _flags, positional) {
  ...
  // wwj skills → list skills for all agents
  if (!first) {
    const agents = connector.config.getAgents();
    if (agents.length === 0) { print('No agents configured'); return; }
    for (const a of agents) {
  ...
  // wwj skills catalog → show full catalog
  if (first === 'catalog') {
  ...
  // wwj skills <agent> enable|disable <skill>
  ...
  if (!third) {
    print(`Usage: wwj skills ${first} ${second} <skill>`);
    print('Available skills: ' + toggleable.map(s => s.id).join(', '));
    process.exitCode = 1;
    return;
  }
...
async function cmdTestLLM(connector, _flags, positional) {
  const type = positional[0];
  if (!type) { print('Usage: wwj test-llm <type>'); return; }
...
async function cmdHelp() {
  print(`Usage: wwj <command> [options]
...
async function main() {
  ...
  const skipUpdateCheck =
    ...
    process.env.WWJ_SKIP_UPDATE_CHECK === '1' ||
  ...
  const commands = {
    ...
    'mcp-server': () => {
      ...
      const workspaceId = flags['workspace-id'] || process.env.WWJ_WORKSPACE_ID;
      const channelName = flags['channel-name'] || process.env.WWJ_CHANNEL_NAME || 'general';
